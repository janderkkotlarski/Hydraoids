...
void limit_hydras(std::vector <hydra>& hydra_objects, int& hydra_count)
{
	
	const int max_count{50};
	assert(max_count > 0);
	
	std::vector <hydra> temp_objects;

	while (hydra_count > max_count)
	{
		
		--hydra_count;
		
		for (int count{0}; count < hydra_count; ++ count)
		{
			temp_objects.push_back(hydra_objects[count + 1]);
		}
		
		for (int count{0}; count < hydra_count + 1; ++ count)
		{
			hydra_objects.pop_back();
		}
		
		for (int count{0}; count < hydra_count; ++ count)
		{
			hydra_objects.push_back(temp_objects[count]);
		}
	...
	while (window.isOpen())
    {
		...
		while(true)
		{
			...
			while (alive)
			{
				...
				if (hydra_count > 0)
				{
					limit_hydras(hydra_objects, hydra_count);
					confirm_hydras(hydra_objects, hydra_count);
				}
